...
def log_usage(start_time, emls_scanned, duplicates):
    '''Log start time, run time, emls scanned, and duplicates removed'''
    file_name = "usage_log.csv"
    run_time = datetime.now() - start_time
    if path.isfile(file_name):
        with open(file_name, "a", encoding='UTF-8') as file:
            file.write(f"{start_time},{run_time},{emls_scanned},{duplicates}\n")
    else:
        with open(file_name, "x", encoding='UTF-8') as file:
            file.write("start time,run time,emls scanned,duplicates\n")
            file.write(f"{start_time},{run_time},{emls_scanned},{duplicates}\n")
...
def main():
    ...
    emls_scanned, duplicates = remove_duplicates(directory)
    print(f"Removed {duplicates} duplicate .eml file{"s" if duplicates != 1 else ""}")
    log_usage(start_time, emls_scanned, duplicates)
